...
import { addCartLine, completeQuickOrder, createCart, ensureBulgariaCommerce, getMedusaProduct, medusaConfigured, startStripeCheckout, type MedusaProduct } from "../lib/medusa"
...
export default function ProductDetail({ slug, initialProduct }: { slug: string; initialProduct?:MedusaProduct|null }) {
  ...
  const submitQuickOrder=async(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();const form=new FormData(event.currentTarget);const variantId=String(form.get("variant")||selectedVariantId);if(!variantId){setQuickSuccess("Моля, изберете размер.");return}setQuickBusy(true);setQuickSuccess("");try{await ensureBulgariaCommerce();const cart=await createCart();await addCartLine(cart.id,variantId);const payment=String(form.get("payment"));const order=await completeQuickOrder(cart.id,{email:String(form.get("email")),name:String(form.get("name")),phone:String(form.get("phone")),address:String(form.get("address")),color:product.color,size:String(form.get("size")),payment,notes:String(form.get("notes")||""),product:product.name});if(payment==="Плащане с карта"){setQuickSuccess("Прехвърляме ви към защитеното плащане на Stripe…");window.location.assign(await startStripeCheckout(order.id));return}setQuickSuccess(`Поръчка${order.display_id?` №${order.display_id}`:"та"} е приета. Плащане: наложен платеж.`)}catch(error){console.error("Quick order failed",error);setQuickSuccess(error instanceof Error?`Поръчката не беше изпратена: ${error.message}`:"Не успяхме да изпратим поръчката. Обадете ни се на 0894 668 387.")}finally{setQuickBusy(false)}}
